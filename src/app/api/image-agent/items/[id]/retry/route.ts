...
export async function POST(
  ...
  try {
    ...
    // Insert NEW item with attempt_count = 0, preserving old failed record for history
        ...
        return NextResponse.json(
          {
            ...
            newBatchId,
            newItemId,
            warning: "تعذر بدء عملية إعادة المحاولة، يرجى المحاولة مرة أخرى",
          },
          { status: 202 }
        );
      }
    } catch (whErr) {
      console.error("n8n retry webhook call failed:", whErr);
      return NextResponse.json(
        {
          success: true,
          newBatchId,
