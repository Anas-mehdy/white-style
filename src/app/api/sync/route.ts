...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(webhookUrl, { method: "POST", headers: { "content-type": "application/json", "x-white-style-sync-secret": secret }, body: JSON.stringify({ source: "dashboard", request_id: requestId }), signal: controller.signal, cache: "no-store" });
